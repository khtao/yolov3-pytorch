...
def create_modules(module_defs):
    ...
    for i, module_def in enumerate(module_defs):
        ...
        if module_def['type'] == 'convolutional':
            ...
            anchors = [int(float(x) * img_width / 416) for x in module_def["anchors"].split(",")]
            anchors = [(anchors[i], anchors[i + 1]) for i in range(0, len(anchors), 2)]
            anchors = [anchors[i] for i in anchor_idxs]

            ignore_thresh = float(module_def['ignore_thresh'])
            # Define detection layer
            yolo_layer = YOLOLayer(anchors, num_classes, [img_height, img_width], ignore_thresh)
            yolo_loss.append(yolo_layer)
    ...
    return hyperparams, module_list, yolo_loss
...
class YOLOLayer(nn.Module):
    ...
    def __init__(self, anchors, num_classes, img_size, ignore_thres=0.5):
    ...
    def forward(self, x, targets=None):
        ...
        if targets is not None:
            ...
            nGT, mask, conf_mask, tx, ty, tw, th, tconf, tcls = build_targets(targets.cpu().data,
                                                                              scaled_anchors,
                                                                              self.num_anchors,
                                                                              self.num_classes,
                                                                              g_dim,
            ...
            loss_conf = self.lambda_cls * self.bce_loss(conf * conf_mask, tconf * conf_mask)
            loss_cls = self.lambda_cls * self.bce_loss(pred_cls * cls_mask, tcls * cls_mask)
...
class Darknet(nn.Module):
    ...
    def __init__(self, config_path, img_size=416):
        ...
        self.hyperparams, self.module_list, self.yolo_loss = create_modules(self.module_defs)
    ...
    def load_pretrain(self, weights_path):
        ...
        self.load_weights(weights_path, only_load_layer=80)

    def load_weights(self, weights_path, only_load_layer=None):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs, self.module_list)):
            ...
            if only_load_layer is not None:
                if i == only_load_layer:
                    break
    ...
    def save_weights(self, path, cutoff=-1):
        ...
        for i, (module_def, module) in enumerate(zip(self.module_defs[:cutoff], self.module_list[:cutoff])):
            if module_def['type'] == 'convolutional':
                ...
                if module_def['batch_normalize']:
                    bn_layer = module[1]
                    bn_layer.bias.data.cpu().numpy().astype(np.float32).tofile(fp)
                    bn_layer.weight.data.cpu().numpy().astype(np.float32).tofile(fp)
                    bn_layer.running_mean.data.cpu().numpy().astype(np.float32).tofile(fp)
                    bn_layer.running_var.data.cpu().numpy().astype(np.float32).tofile(fp)
                # Load conv bias
                else:
                    conv_layer.bias.data.cpu().numpy().astype(np.float32).tofile(fp)
                # Load conv weights
                conv_layer.weight.data.cpu().numpy().astype(np.float32).tofile(fp)
